...
from year_ap_predictive.predictive_framework import to_numpy_2d_matrix


def plot_and_show(grouped_data: GroupDataSet, models: GroupModels, scores: GroupScores, xlabel: str, ylabel: str) -> None:
    """
    Plots the real data and the learned curve. Assumes both feature and value vector are univariate.
    """
    n_groups = len(models.keys())
    _, ax = plt.subplots(n_groups, 1)
    ...
    for i, p in enumerate(models.items()):
        ...
        ax[i].plot(X, Y, 'bo', label='sample data points')
        ax[i].plot(test_x, test_y, 'r-', label='learned line')
        ax[i].plot(test_x, test_x, 'k--', label='y=x')
